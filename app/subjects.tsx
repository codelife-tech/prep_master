...
export default function SubjectsScreen() {
    ...
    if (!examData) return <View style={styles.container}><Text style={styles.errorText}>Exam not found</Text></View>;
        ...
        <SafeAreaView style={styles.container}>
            <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
                <View style={styles.header}>
                    <Text style={styles.examType}>{examData.type}</Text>
                    <Text style={styles.subtitle}>{examData.description}</Text>
                    <Text style={styles.totalBadge}>{totalQuestions} total questions</Text>
                </View>
...
const styles = StyleSheet.create({
    ...
    examType: { fontSize: FontSize.xxl, fontWeight: '900', color: Colors.primary },
    subtitle: { fontSize: FontSize.sm, color: Colors.textSecondary, marginTop: Spacing.xs, textAlign: 'center' },
    totalBadge: { fontSize: FontSize.xs, color: Colors.accent, marginTop: Spacing.sm, fontWeight: '600' },
